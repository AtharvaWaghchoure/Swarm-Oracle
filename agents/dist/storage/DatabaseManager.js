...
export class DatabaseManager {
    ...
    async getMemories(agentId, limit) {
        if (!this.connected)
            await this.connect();
        if (this.fallbackMode) {
    ...
    async getMemoriesByType(agentId, type, limit) {
        if (!this.connected)
            await this.connect();
        if (this.fallbackMode) {
            const memories = Array.from(this.memoryStorage.values())
                .filter((m) => m.agentId === agentId && m.type === type)
                .sort((a, b) => b.timestamp - a.timestamp);
            return limit ? memories.slice(0, limit) : memories;
        }
        if (!this.pool)
    ...
    async storePrediction(marketId, agentSwarmId, prediction, confidence) {
        if (!this.connected)
            await this.connect();
        if (this.fallbackMode) {
    ...
    async getPredictions(marketId, limit) {
        if (!this.connected)
            await this.connect();
        if (this.fallbackMode) {
    ...
    async cleanup(olderThanTimestamp) {
        ...
        if (this.fallbackMode) {
            ...
            for (const [key, value] of this.memoryStorage.entries()) {
                if (value.timestamp && value.timestamp < olderThanTimestamp) {
    ...
    async storeAgentRegistration(agentInfo) {
        if (!this.connected) {
    ...
    async getMessages(agentId, limit = 10) {
        ...
        if (agentId) {
            messages = messages.filter(msg => msg.fromAgentId === agentId || msg.toAgentId === agentId);
        }
        return messages
            .slice(-limit)
            .sort((a, b) => b.timestamp - a.timestamp);
