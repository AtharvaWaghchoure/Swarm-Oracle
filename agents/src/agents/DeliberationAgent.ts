...
export class DeliberationAgent extends BaseAgent {
  private deliberationType: string;
  private role: string;
  private deliberationConfig: any;
   ...
   */
  private async deliberate(proposals: any[], consensusThreshold: number): Promise<any> {
  ...
  private async facilitateConsensus(task: any): Promise<any> {
    ...
    return {
      ...
      participatingAgents: predictions.map((p: any) => p.agentId),
      reasoning: this.explainConsensus(analysis, consensus)
    };
  }

  private async resolveDispute(task: any): Promise<any> {
    ...
    return {
      ...
      conflictingAgents: conflictingPredictions.map((p: any) => p.agentId),
      resolution,
      reasoning: this.explainResolution(resolution, decision)
    };
  }

  private analyzePredictions(predictions: any[]): any {
    ...
    for (const prediction of predictions) {
      if (prediction.prediction && prediction.prediction.direction) {
        directions[prediction.prediction.direction as keyof typeof directions]++;
  ...
  private calculateConsensus(analysis: any, threshold: number): any {
    ...
    const consensusRatio = dominantDirection[1] as number / total;
  ...
  private analyzeConflict(conflictingPredictions: any[], evidence: any[]): any {
    const conflicts = [];
    const evidenceWeights: { [key: string]: number } = {};
  ...
  private makeResolutionDecision(resolution: any): any {
    // Simple resolution strategy - can be made more sophisticated
    const hasStrongEvidence = Object.values(resolution.evidenceWeights).some((weight: any) => weight > 0.8);
    const conflictSeverity = resolution.conflicts.reduce((sum: number, c: any) => sum + c.severity, 0) / resolution.conflicts.length;
  ...
  private calculateEvidenceWeight(evidence: any): number {
    let weight = 0.5; // Base weight
    
    // Increase weight based on source reliability
    if (evidence.source === 'onchain') weight += 0.3;
    if (evidence.source === 'news') weight += 0.2;
    if (evidence.source === 'social') weight += 0.1;
    
    // Adjust for recency
    const ageHours = (Date.now() - evidence.timestamp) / (1000 * 60 * 60);
    if (ageHours < 1) weight += 0.2;
    else if (ageHours < 24) weight += 0.1;
    
    return Math.min(1, weight);
  }

  private explainConsensus(analysis: any, consensus: any): string {
  ...
  private explainResolution(resolution: any, decision: any): string {
